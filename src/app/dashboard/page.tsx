...
export default function Dashboard() {
  ...
  // Download laporan untuk satu assessment (per-row)
  const downloadAssessmentReport = (a: Assessment) => {
    const questions = [
      'Incomplete Emptying',
      'Frequency',
      'Intermittency',
    ...
    let content = `LAPORAN ASESMEN INDIVIDU\n\n`;
    content += `ID: ${a.id}\nTanggal: ${new Date(a.date).toLocaleString('id-ID')}\nTotal Skor: ${a.total_score}\nQoL: ${a.qol}\nKategori: ${a.category}\n\nRincian Jawaban:\n`;

    const answers = [a.q1, a.q2, a.q3, a.q4, a.q5, a.q6, a.q7];
    questions.forEach((q, idx) => {
      content += `${idx + 1}. ${q}: ${answers[idx]}\n`;
    });

    content += `\nCatatan: ${ (a as any).notes || '-' }\n`;

    const blob = new Blob([content], { type: 'text/plain' });
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `Laporan_Asesmen_${a.id}_${new Date(a.date).toISOString().split('T')[0]}.txt`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
                        ...
                        <td className="text-center py-3 px-4">
                          <div className="flex items-center justify-center gap-2">
                            <button 
                              ...
                              className="flex items-center mx-auto space-x-1 text-blue-600 hover:text-blue-800 font-medium text-sm border border-blue-200 px-3 py-1 rounded-lg hover:bg-blue-50 transition-all"
                            >
                              <Eye size={16} />
                            ...
                            <button
                              ...
                              className="flex items-center mx-auto space-x-1 text-green-600 hover:text-green-800 font-medium text-sm border border-green-200 px-3 py-1 rounded-lg hover:bg-green-50 transition-all"
                            >
                              <Download size={16} />
