...
export default function Navigation() {
  ...
  useEffect(() => {
    setIsMounted(true);
    // Cek LocalStorage (Tempat penyimpanan browser)
  ...
  // Listen for profile updates from other parts of the app (same-tab via custom event)
  useEffect(() => {
  ...
  // 2. Fungsi Logout (Fitur yang kamu minta)
  const handleLogout = () => {
    ...
    // Redirect (Pindahkan) ke halaman login
    router.push('/login');
    // Refresh agar halaman benar-benar bersih dari data lama
    router.refresh();
  };

  // Mencegah error tampilan saat loading pertama kali
  if (!isMounted) return null;

  // Jangan tampilkan Navigasi di halaman Landing (/) serta Login, Register, dan IPSS
    ...
    <nav className="bg-white shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* LOGO or Back Button on Edukasi */}
          {isEdukasi ? (
            <button
              onClick={() => {
                try {
          ...
          <div className="hidden md:flex items-center space-x-8">
            <Link 
              href="/dashboard" 
              className={`flex items-center space-x-1 hover:text-blue-600 transition ${pathname === '/dashboard' ? 'text-blue-600 font-semibold' : 'text-gray-600'}`}
            >
              <Home size={18} />
              <span>Dashboard</span>
            </Link>
            ...
            <Link 
              href="/ipss" 
              className={`flex items-center space-x-1 hover:text-blue-600 transition ${pathname === '/ipss' ? 'text-blue-600 font-semibold' : 'text-gray-600'}`}
            >
              <FileText size={18} />
              <span>Asesmen</span>
            </Link>
            ...
            <Link 
              href="/edukasi" 
              className={`flex items-center space-x-1 hover:text-blue-600 transition ${pathname === '/edukasi' ? 'text-blue-600 font-semibold' : 'text-gray-600'}`}
            >
              <Book size={18} />
              <span>Edukasi</span>
            </Link>
            ...
            <div className="border-l border-gray-300 pl-6 ml-2 flex items-center">
              {userName ? (
        ...
        <div className="md:hidden bg-white border-t border-gray-100">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            <div className="border-t border-gray-200 mt-4 pt-4">
              {userName ? (
                <div className="px-3">
                  <Link href="/profile" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center mb-3 space-x-3">
                    {userAvatar ? (
                      <img src={userAvatar} alt="avatar" className="w-10 h-10 rounded-full object-cover" />
                    ) : (
                      <div className="bg-blue-100 p-2 rounded-full text-blue-600 mr-0">
                        <User size={20} />
                      </div>
                    )}
                    <div>
